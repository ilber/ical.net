...
namespace Ical.Net.Interfaces.Evaluation
{
	public interface IGetOccurrences
	{
		...
		HashSet<IOccurrence> GetOccurrences(IDateTime dt);

		HashSet<IOccurrence> GetOccurrences(DateTime dt);
		...
		HashSet<IOccurrence> GetOccurrences(IDateTime startTime, IDateTime endTime);

		HashSet<IOccurrence> GetOccurrences(DateTime startTime, DateTime endTime);
	}
	...
	public interface IGetOccurrencesTyped : IGetOccurrences
	{
		...
		/// <param name="dt">The date for which to return occurrences.</param>
		/// <returns>A list of Periods representing the occurrences of this object.</returns>
		HashSet<IOccurrence> GetOccurrences<T>(IDateTime dt) where T : IRecurringComponent;

		HashSet<IOccurrence> GetOccurrences<T>(DateTime dt) where T : IRecurringComponent;
		...
		/// <param name="startTime">The starting date range</param>
		/// <param name="endTime">The ending date range</param>
		HashSet<IOccurrence> GetOccurrences<T>(IDateTime startTime, IDateTime endTime) where T : IRecurringComponent;

		HashSet<IOccurrence> GetOccurrences<T>(DateTime startTime, DateTime endTime) where T : IRecurringComponent;
	}
}